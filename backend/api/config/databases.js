...
const connectToDatabase1 = async() => {
    try {
        ...
        const conn1 = await mongoose.createConnection(process.env.DATABASE_URI1, { useNewUrlParser: true, useUnifiedTopology: true });
...
const connectToDatabase2 = async() => {
    try {
        mongoose.set('strictQuery', false);
        const conn2 = await mongoose.createConnection(process.env.DATABASE_URI2, { useNewUrlParser: true, useUnifiedTopology: true });
