...
exports.createHospital = async (req, res) => {
    try {
        const existingHospital = await HospitalModel.findOne({ address: req.body.address, phone: req.body.phone });
        if (existingHospital) {
            return res.status(400).json({ message: 'Hospital with this address and phone number already exists' });
        }
